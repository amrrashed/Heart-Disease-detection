...
# Merge the data
train_data = np.concatenate((X_train_encode, np.expand_dims(y_train, axis=1)), axis=1)
test_data = np.concatenate((X_test_encode, np.expand_dims(y_test, axis=1)), axis=1)

# Combine train and test data
combined_data = np.concatenate((train_data, test_data), axis=0)

# Convert to DataFrame
columns = [f"encoded_{i}" for i in range(X_train_encode.shape[1])] + ['class']
combined_df = pd.DataFrame(combined_data, columns=columns)

# Save to CSV
combined_df.to_csv("D:/new researches/Heart disease/M4 AUTOENCODER/encodedM4_statlog.csv", index=False)
